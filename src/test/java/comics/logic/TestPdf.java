...
public class TestPdf {
    ...
    @SandboxTest
    public void testCommand(Sandbox sb) throws Exception {
        var pdf = sb.copyResource("/compressed/test.pdf", "test.pdf");
        var command = new Pdf2CbzCommand();
        command.setDisableProgressBar(true);
        var ret = command.run(sb.getSandbox().toPath());
        assertEquals(0, ret);
        assertFalse(pdf.exists());
        // We have created a directory as intermediate step that should not be there any more
        var intermediateDirectory = new File(sb.getSandbox(), "test");
        assertFalse(intermediateDirectory.exists());
        // Result
        var expectedFile = new File(sb.getSandbox(), "Test.cbz");
        assertTrue(expectedFile.exists());
        assertFalse(expectedFile.isDirectory());
        new CompressionService().decompressComic(expectedFile);
        checkAllImages(new File(sb.getSandbox(), "Test"), DEFAULT_FORMAT);
    }

    @SandboxTest
    public void testCommandPng(Sandbox sb) throws Exception {
        var format = "png";
        var pdf = sb.copyResource("/compressed/test.pdf", "test.pdf");
        ...
        assertFalse(expectedFile.isDirectory());
        new CompressionService().decompressComic(expectedFile);
        checkAllImages(new File(sb.getSandbox(), "Test"), format);
    }
}
