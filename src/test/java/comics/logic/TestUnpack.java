...
public class TestUnpack {
    ...
    @SandboxTest
    public void testCommandErrorExistingDirectory(Sandbox sb) throws IOException {
        var targetFile = sb.copyResource("/compressed/test.cbr", "test.cbr");
        var obstacle = new File(sb.getSandbox(), "test");
    ...
    @SandboxTest
    public void testCommandErrorExistingFile(Sandbox sb) throws IOException {
    ...
    @SandboxTest
    public void testCommandErrorNullDirectory(Sandbox sb) {
        var ctx = captureOutput(() -> {
            var command = new UnpackCommand();
            command.setDisableProgressBar(TRUE);
    ...
    public void testServiceErrors(Sandbox sb) throws IOException {
        ...
        var newDir = new File(sb.getSandbox(), "childDir");
        mkdir(newDir);
        assertThrowsExactly(CompressionException.class, () -> service.decompressComic(newDir));
        // non existing file
        assertThrowsExactly(CompressionException.class, () -> service.decompressComic(new File(sb.getSandbox(), "lalala")));
    }
